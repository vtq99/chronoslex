...
PREPROCESSED_FILE = 'eurlex127.pkl'
MAX_TOKEN_LENGTH = 512
RAW_DATA_FILE = 'eurlex127.jsonl'
ID_HELD_OUT = 0.2
GROUP = 2

class EURLex127Base(Dataset):
    def __init__(self, args):
        ...
        download_detection(args.data_dir, RAW_DATA_FILE)
        preprocess(args)

        self.datasets = pickle.load(open(os.path.join(args.data_dir, self.data_file), 'rb'))

        self.args = args
        self.ENV = [1985, 1987, 1989, 1991, 1993, 1995, 1997, 1999, 2001, 2003, 2005, 2007, 2009, 2011, 2013, 2015]
        # self.ENV = [2001, 2003, 2005, 2007, 2009, 2011, 2013, 2015]
...
class EURLex127Group(EURLex127Base):
    def __init__(self, args):
        super().__init__(args=args)
...
def preprocess_orig(args):
    ...
    all_dfs[-2] = pd.concat([all_dfs[-2], all_dfs[-1]])
    all_dfs = all_dfs[:-1]
    ...
    if GROUP < 2:
        ...
        dfs.append(pd.concat(all_dfs[:24]))
        years.append(all_years[23])
        all_dfs = all_dfs[24:]
        all_years = all_years[24:]
