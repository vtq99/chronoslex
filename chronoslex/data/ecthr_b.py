...
class ECtHRBBase(Dataset):
    def __init__(self, args):
        ...
        self.num_classes = 17 # 15 if we remove classes
...
"""
Categories to IDs:
    {'10': 0, '11': 1, '13': 2, '14': 3, '18': 4, '2': 5, '3': 6, '4': 7, '5': 8, '6': 9, '7': 10, '8': 11, '9': 12,
    'P1-1': 13, 'P4-2': 14, 'P7-1': 15, 'P7-4': 16}
    
IDs to Categories:
    {0: '10', 1: '11', 2: '13', 3: '14', 4: '18', 5: '2', 6: '3', 7: '4', 8: '5', 9: '6', 10: '7', 11: '8', 12: '9',
    13: 'P1-1', 14: 'P4-2', 15: 'P7-1', 16: 'P7-4'}
"""
...
def preprocess_orig(args):
    ...
    categories_to_classids = {category: classid for classid, category in
                              enumerate(sorted(list(set([i for sublist in base_df['allegedly_violated_articles'] for i in sublist]))))}
    ...
    for i, year in enumerate(years):
        ...
        for j in df_year.index:
            new_label = []
            for label in df_year.at[j, 'allegedly_violated_articles']:
                if label in categories_to_classids.keys():
                    new_label.append(label)
            df_year.at[j, 'allegedly_violated_articles'] = new_label

        headlines = df_year['facts'].tolist()
        categories = []
        for label in df_year['allegedly_violated_articles']:
            new_label = []
            for l in label:
